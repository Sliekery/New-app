...
(function (ns) {
  ...
  function drawPlayerGauge(px, py, scale) {
    ...
    var capW = h * 1.7, barW = scale * 3.7;
    var bx = px - scale * 1.2;                   // left edge of the energy cap
    var barX = bx + capW - h * 0.4, barEnd = barX + barW;   // HP bar tucks behind the cap
    var sk = h * 0.42;                           // slant amount
    var blueFill = '#2f9fd6', blueLine = '#7fe0ff';
    ctx.save();
    ctx.lineJoin = 'miter'; ctx.lineCap = 'butt';

    // ===== 1. GREEN HP BAR — sits behind the energy cap & shield bar =====
    function barPath() {
      ctx.beginPath();
      ctx.moveTo(barX + sk, by); ctx.lineTo(barEnd, by); ctx.lineTo(barEnd - sk, by + h);
      ctx.lineTo(barX, by + h); ctx.closePath();
    }
    barPath(); ctx.fillStyle = 'rgba(4,12,14,0.92)'; ctx.fill();
    ctx.save(); barPath(); ctx.clip();
    var fillW = barW * frac;
    var hpCol = frac > 0.5 ? '#5dff88' : frac > 0.25 ? '#ffb02e' : '#ff4a5e';
    ctx.fillStyle = hpCol; ctx.globalAlpha = 0.32; ctx.fillRect(barX, by, fillW + sk, h);
    ...
    // ===== 2. SHIELD BAR — solid (opaque), segmented, overlaps the lower HP
    //         bar and peeks below; grows / shrinks with shield =====
    if (block > 0) {
      var shTop = by + h * 0.5, shH = h * 0.74;
      var sw = Math.max(h * 0.9, Math.min(1, block / maxHp) * barW);   // capped at HP width
      var shPath = function (w) {
        ctx.beginPath();
        ctx.moveTo(barX + sk, shTop); ctx.lineTo(barX + w + sk, shTop);
        ctx.lineTo(barX + w, shTop + shH); ctx.lineTo(barX, shTop + shH); ctx.closePath();
      };
      ctx.save(); shPath(sw); ctx.clip();
      ctx.fillStyle = blueFill; ctx.fillRect(barX - sk, shTop, sw + sk * 2 + 2, shH);  // solid: hides HP behind it
      ctx.strokeStyle = 'rgba(6,22,34,0.6)'; ctx.lineWidth = 1.6;                       // segment dividers
      for (var vx = barX + 11; vx < barX + sw; vx += 11) {
        ctx.beginPath(); ctx.moveTo(vx + sk, shTop); ctx.lineTo(vx, shTop + shH); ctx.stroke();
      }
      ctx.restore();
      shPath(sw); ctx.strokeStyle = blueLine; ctx.lineWidth = 1.5; ctx.shadowColor = '#41d8ff'; ctx.shadowBlur = 6; ctx.stroke(); ctx.shadowBlur = 0;
    }

    // ===== 3. ENERGY CAP — solid blue, taller than the HP bar, on top, left =====
    var capH = h * 1.34, capTop = by - (capH - h) / 2, csk = capH * 0.42;
    ctx.beginPath();
    ctx.moveTo(bx + csk, capTop); ctx.lineTo(bx + capW, capTop); ctx.lineTo(bx + capW + csk, capTop + capH);
    ctx.lineTo(bx, capTop + capH); ctx.closePath();
    ctx.fillStyle = blueFill; ctx.fill();
    ctx.strokeStyle = blueLine; ctx.lineWidth = 1.6; ctx.shadowColor = '#41d8ff'; ctx.shadowBlur = 7; ctx.stroke();
    ctx.shadowBlur = 0;

    // ===== 4. NUMBERS (on top of everything) =====
    ctx.textBaseline = 'middle';
    ctx.fillStyle = '#f2fdff'; ctx.shadowColor = '#04121c'; ctx.shadowBlur = 2;
    ctx.font = 'bold ' + Math.round(capH * 0.6) + 'px monospace'; ctx.textAlign = 'center';
    ctx.fillText('' + energy, bx + capW * 0.52 + csk * 0.5, capTop + capH / 2 + 0.5);
    // HP number — ride high so the shield bar doesn't cover it
    var hpNumX = (Math.max(barX, bx + capW) + barEnd) / 2;
    ctx.fillStyle = '#eaffef'; ctx.shadowColor = '#000'; ctx.shadowBlur = 3;
    ctx.font = 'bold ' + Math.round(h * 0.5) + 'px monospace';
    ctx.fillText(hp + ' / ' + maxHp, hpNumX, by + h * (block > 0 ? 0.32 : 0.5) + 0.5);
    ctx.shadowBlur = 0;
    // shield number, in the shield bar
    if (block > 0) {
      ctx.fillStyle = '#f2fdff'; ctx.shadowColor = '#04121c'; ctx.shadowBlur = 2;
      ctx.font = 'bold ' + Math.round(h * 0.48) + 'px monospace'; ctx.textAlign = 'left';
      ctx.fillText('⬡' + block, Math.max(barX + sk + 4, bx + capW + 3), by + h * 0.87);
      ctx.shadowBlur = 0;
    }
    ctx.restore();
  }
