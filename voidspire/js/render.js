...
(function (ns) {
  ...
  function drawPlayerGauge(px, py, scale) {
    ...
    // --- dynamic shield bar: hatched, layered BEHIND the HP bar and offset
    //     down so it peeks out beneath it (overlapping plates) ---
    if (block > 0) {
      var shOffY = h * 0.5;
      var sw = Math.min(1, block / maxHp) * barW;   // graphic caps at the HP bar width
      var shPath = function (w) {
        ctx.beginPath();
        ctx.moveTo(barX + sk, by + shOffY); ctx.lineTo(barX + w + sk, by + shOffY);
        ctx.lineTo(barX + w, by + shOffY + h); ctx.lineTo(barX, by + shOffY + h); ctx.closePath();
      };
      ctx.save(); shPath(sw); ctx.clip();
      ctx.fillStyle = 'rgba(65,216,255,0.18)'; ctx.fillRect(barX, by + shOffY, sw + sk + 2, h);
      ctx.strokeStyle = 'rgba(65,216,255,0.9)'; ctx.lineWidth = 1.1;
      for (var sx = barX - h; sx < barX + sw + h; sx += 5) {
        ctx.beginPath(); ctx.moveTo(sx + sk, by + shOffY); ctx.lineTo(sx - h + sk, by + shOffY + h); ctx.stroke();
      }
      ctx.restore();
    ...
    // shield value — sits in the peeking strip; number keeps climbing past the cap
    if (block > 0) {
      ctx.fillStyle = '#dff6ff'; ctx.shadowColor = '#031018'; ctx.shadowBlur = 3;
      ctx.font = 'bold ' + Math.round(h * 0.48) + 'px monospace'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
      ctx.fillText('⬡' + block, barX + sk + 4, by + h * 1.25);
      ctx.shadowBlur = 0;
    }
    ctx.restore();
  }
