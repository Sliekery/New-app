...
(function (ns) {
  ...
  R.frame = function (dt) {
    ...
    if (R.combatVisible) drawPowerGauge();   // augmentation meter sits on top
  };
  ...
  function drawPlayerGauge(px, py, scale) {
    ...
    var by = py + scale * 1.66;                  // top of the HP bar
    var capW = h * 1.12;                          // energy box
    var bx = px - scale * 1.18;                   // left edge of the energy box
    var barX = bx + capW + h * 0.5, barW = scale * 3.5, barEnd = barX + barW;
    var GREEN = '#5dff88', GREEN_DIM = '#43806a', CYAN = '#41d8ff';
    var hpCol = frac > 0.5 ? GREEN : frac > 0.25 ? '#ffb02e' : '#ff4a5e';
    ctx.save();
    ctx.lineJoin = 'miter'; ctx.lineCap = 'butt'; ctx.textBaseline = 'middle';

    // ===== ENERGY — small outlined box with an up-arrow, far left =====
    ctx.strokeStyle = CYAN; ctx.lineWidth = 1.5; ctx.shadowColor = CYAN; ctx.shadowBlur = 5;
    ctx.strokeRect(bx, by, capW, h);
    ctx.shadowBlur = 0;
    var axc = bx + capW / 2, atop = by - h * 0.36;          // arrow above the box
    ctx.beginPath();
    ctx.moveTo(axc, atop); ctx.lineTo(axc - h * 0.24, atop + h * 0.28); ctx.lineTo(axc + h * 0.24, atop + h * 0.28); ctx.closePath();
    ctx.fillStyle = CYAN; ctx.fill();
    ctx.fillStyle = '#eafcff'; ctx.font = 'bold ' + Math.round(h * 0.66) + 'px monospace'; ctx.textAlign = 'center';
    ctx.fillText('' + energy, bx + capW / 2, by + h / 2 + 0.5);

    // ===== HP BAR — clean outline + number + thin progress underline =====
    ctx.strokeStyle = GREEN_DIM; ctx.lineWidth = 1.5; ctx.shadowColor = 'rgba(93,255,136,0.35)'; ctx.shadowBlur = 4;
    ctx.strokeRect(barX, by, barW, h);
    ctx.shadowBlur = 0;
    ctx.strokeStyle = hpCol; ctx.lineWidth = 2.4; ctx.shadowColor = hpCol; ctx.shadowBlur = 5;
    ctx.beginPath(); ctx.moveTo(barX + 1.5, by + h - 1.6); ctx.lineTo(barX + 1.5 + (barW - 3) * frac, by + h - 1.6); ctx.stroke();
    ctx.shadowBlur = 0;
    ctx.fillStyle = '#eaffef'; ctx.shadowColor = '#000'; ctx.shadowBlur = 3;
    ctx.font = 'bold ' + Math.round(h * 0.56) + 'px monospace'; ctx.textAlign = 'center';
    ctx.fillText(hp + ' / ' + maxHp, (barX + barEnd) / 2, by + h * 0.46);
    ctx.shadowBlur = 0;

    // ===== SHIELD — hexagon value + a hatched "vector line" beneath the HP bar
    //       (lines only, no fill, no overlap with the HP bar) =====
    if (block > 0) {
      var r = h * 0.36, hyc = by + h + r + h * 0.16, hxc = barX + r;
      // hexagon node holding the value
      ctx.beginPath();
      for (var i = 0; i < 6; i++) { var a = Math.PI / 6 + i * Math.PI / 3, hx = hxc + r * Math.cos(a), hy = hyc + r * Math.sin(a); i ? ctx.lineTo(hx, hy) : ctx.moveTo(hx, hy); }
      ctx.closePath();
      ctx.strokeStyle = CYAN; ctx.lineWidth = 1.5; ctx.shadowColor = CYAN; ctx.shadowBlur = 5; ctx.stroke(); ctx.shadowBlur = 0;
      ctx.fillStyle = '#eafcff'; ctx.font = 'bold ' + Math.round(r * 1.0) + 'px monospace'; ctx.textAlign = 'center';
      ctx.fillText('' + block, hxc, hyc + 0.5);
      // vector line extending right, length = shield (capped at the HP bar end)
      var ls = hxc + r + 3, maxLen = barEnd - ls;
      var len = Math.min(1, block / maxHp) * maxLen, hh = h * 0.46;
      ctx.strokeStyle = 'rgba(65,216,255,0.95)'; ctx.lineWidth = 1.4; ctx.shadowColor = CYAN; ctx.shadowBlur = 3;
      ctx.beginPath(); ctx.moveTo(ls, hyc); ctx.lineTo(ls + len, hyc); ctx.stroke(); ctx.shadowBlur = 0;
      // hatch ticks along the line, + an end cap
      ctx.lineWidth = 1.1;
      for (var tx = ls + 5; tx < ls + len; tx += 6) {
        ctx.beginPath(); ctx.moveTo(tx, hyc - hh / 2); ctx.lineTo(tx - hh * 0.5, hyc + hh / 2); ctx.stroke();
      }
      ctx.beginPath(); ctx.moveTo(ls + len, hyc - hh / 2); ctx.lineTo(ls + len, hyc + hh / 2); ctx.stroke();
    }
    ctx.restore();
  }

  /* ---- power gauge: active Powers as a stacked "battle augmentation" meter,
   *      tap to expand a docked tactical readout (names + values) ----------- */
  var readoutOpen = false, gaugeBox = null, readoutBox = null;
  // category -> colour, so the gauge reads like a varied console (offense /
  ...
  function activePowers() {
    ...
    Object.keys(st).forEach(function (k) {
      ...
      var cat = POWER_CAT[k] || 'sp';
      out.push({ name: (ns.STATUS_NAMES[k] || k), val: st[k], color: CAT_COL[cat] });
    });
    return out;
  }

  function bracket(x, y, dx, dy, n) {
    ctx.beginPath(); ctx.moveTo(x, y + dy * n); ctx.lineTo(x, y); ctx.lineTo(x + dx * n, y); ctx.stroke();
  }
  ...
  function drawReadout(powers, gx, gy, p) {
    ...
    powers.forEach(function (pw, i) {
      ...
      ctx.fillStyle = '#cfeaf5'; ctx.font = '9px monospace'; ctx.fillText(pw.name.toUpperCase(), x + 22, ry);
      ctx.fillStyle = pw.color; ctx.textAlign = 'right'; ctx.fillText('' + pw.val, x + w - 9, ry); ctx.textAlign = 'left';
    });
    ctx.restore();
    readoutBox = { x: x, y: y, w: w, h: h };
  }
  ...
  function drawPowerGauge() {
    ...
    ctx.save();
    for (var i = 0; i < n; i++) {
      var syy = gy + (n - 1 - i) * (sq + gap);            // i=0 sits at the bottom; stacks upward
      ctx.globalAlpha = pulse * (i === n - 1 ? 1 : 0.85);
      ctx.fillStyle = powers[i].color; ctx.shadowColor = powers[i].color; ctx.shadowBlur = 5;
      ctx.fillRect(gx, syy, sq, sq);
    }
    ctx.shadowBlur = 0; ctx.globalAlpha = 0.45; ctx.strokeStyle = '#7fd8ff'; ctx.lineWidth = 1;
    ctx.strokeRect(gx - 2.5, gy - 2.5, sq + 5, totalH + 5);
    ctx.restore();
    gaugeBox = { x: gx - 8, y: gy - 8, w: sq + 16, h: totalH + 16 };  // generous tap target
    if (readoutOpen) drawReadout(powers, gx, gy, p); else readoutBox = null;
  }

  R.gaugeHit = function (x, y) { return !!gaugeBox && x >= gaugeBox.x && x <= gaugeBox.x + gaugeBox.w && y >= gaugeBox.y && y <= gaugeBox.y + gaugeBox.h; };
  R.readoutHit = function (x, y) { return !!readoutBox && x >= readoutBox.x && x <= readoutBox.x + readoutBox.w && y >= readoutBox.y && y <= readoutBox.y + readoutBox.h; };
  R.isReadoutOpen = function () { return readoutOpen; };
  R.toggleReadout = function () { readoutOpen = !readoutOpen; return readoutOpen; };
  R.closeReadout = function () { readoutOpen = false; };
