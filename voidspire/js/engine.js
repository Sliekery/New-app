...
(function (ns) {
   ...
   * ------------------------------------------------------------------- */
  /* VS_NO_DIE is a balance probe, not a game option. It silences the die
   * entirely — no engraving fires, no band multiplies — so the question "is the
   * die adding depth or just adding variance?" can be answered against a
   * control. If drafting matters MORE with the die off, the die is diluting the
   * decisions rather than deepening them. */
  var NO_DIE = (typeof process !== 'undefined' && process.env && !!process.env.VS_NO_DIE);
  /* VS_FLAT_DIE keeps the die's POWER and removes only its VARIANCE: instead of
   * rolling, it walks 1..20 in order, so every face still fires exactly as often
   * over a run and the expected value is identical. VS_NO_DIE could not answer
   * the question — silencing the die dropped the win rate to 8% and at that
   * difficulty every arm converges, which is a floor effect, not a finding.
   * This isolates the one thing actually in question: is the randomness adding
   * depth, or is it drowning the decisions? */
  var FLAT_DIE = (typeof process !== 'undefined' && process.env && !!process.env.VS_FLAT_DIE);
  ...
  function randomEngravings(n) {
    ...
    ids.forEach(function (k) {
      ...
      /* A BASIC IS A FLOOR, NOT A REWARD. Brace and Pulse Rifle are what the
       * dice arrive wearing so a fresh barrel is not six blanks — they are the
