 ...
 * keep the stroke ORDER, because every part of this rig addresses a stroke by
 * its index — which points are pinned, which strokes a stretch carries, which
 * selection a transform is replayed over.
 *
...
(async () => {
  ...
  const JOINT = (await armOf())[1];
  {
    ...
    ok('every frame still has both strokes, in the same order — indices are the rig',
